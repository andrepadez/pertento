...
const byId = topDocument.getElementById.bind(topDocument);
const useIframe = () => {
  const [editorWrapper] = reactExports.useState(() => byId("pertento-editor-iframe-wrapper"));
  const [websiteWrapper] = reactExports.useState(() => byId("pertento-website-iframe-wrapper"));
  const [editorIframe] = reactExports.useState(() => byId("pertento-editor-iframe"));
  const [websiteIframe] = reactExports.useState(() => byId("pertento-website-iframe"));
  const [editorWindow] = reactExports.useState(() => editorIframe == null ? void 0 : editorIframe.contentWindow);
  const [websiteWindow] = reactExports.useState(() => websiteIframe == null ? void 0 : websiteIframe.contentWindow);
  const [editorDocument] = reactExports.useState(() => editorIframe == null ? void 0 : editorIframe.contentDocument);
  const [websiteDocument] = reactExports.useState(() => websiteIframe == null ? void 0 : websiteIframe.contentDocument);
  const sendMessage = (type, payload) => {
    websiteWindow == null ? void 0 : websiteWindow.postMessage({ type, payload }, "*");
  };
  const reload = () => {
...
const useInteractivity = () => {
  const [interactive, setInteractive] = reactExports.useState(true);
  const { sendMessage } = useIframe();
  reactExports.useEffect(() => {
    sendMessage("PERTENTO_TOGGLE_INTERACTIVE", { value: interactive }, "*");
  }, [interactive]);
  return { interactive, setInteractive };
};
const usePointerControl = () => {
  const [pointerController, setPointerController] = reactExports.useState("website");
  const iframeManager = useIframe();
  const { topWindow: topWindow2, editorWrapper, websiteWrapper, editorIframe } = iframeManager;
  const { websiteIframe, editorWindow, websiteWindow, editorDocument } = iframeManager;
  const { websiteDocument, sendMessage } = iframeManager;
  reactExports.useEffect(() => {
    const onKeyDown = (ev) => {
      if (["INPUT", "TEXTAREA"].includes(ev.target.tagName))
        return;
      if (ev.keyCode === 87) {
        changePointerControlTo("website");
      } else if (ev.keyCode === 69) {
        changePointerControlTo("editor");
      }
    };
    topWindow2.addEventListener("keydown", onKeyDown);
    editorIframe == null ? void 0 : editorIframe.contentWindow.addEventListener("keydown", onKeyDown);
    websiteIframe == null ? void 0 : websiteIframe.contentWindow.addEventListener("keydown", onKeyDown);
  }, []);
  const changePointerControlTo = (controller) => {
    if (controller === "website") {
      setPointerController("website");
      editorWrapper.style.pointerEvents = "none";
      editorIframe.style.pointerEvents = "none";
      editorDocument.body.style.pointerEvents = "none";
      websiteWrapper.style.pointerEvents = "auto";
      websiteIframe.style.pointerEvents = "auto";
      websiteDocument.body.style.pointerEvents = "auto";
    } else {
      setPointerController("editor");
      editorWrapper.style.pointerEvents = "auto";
      editorIframe.style.pointerEvents = "auto";
      editorDocument.body.style.pointerEvents = "auto";
      websiteWrapper.style.pointerEvents = "none";
      websiteIframe.style.pointerEvents = "none";
      websiteDocument.body.style.pointerEvents = "none";
    }
  };
  return { pointerController, changePointerControlTo };
};
const EditorFloater = () => {
  const { interactive, setInteractive } = useInteractivity();
  const { pointerController, changePointerControlTo } = usePointerControl();
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inline-block p-2 bg-white border-2 select-none rounded-xl", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-10", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("img", { className: "w-10 h-10 rounded-full", src: "https://app.pertento.ai/logo.png", alt: "logo" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-3", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Button,
        {
          size: "sm",
          variant: pointerController === "editor" ? "default" : "outline",
          onClick: () => changePointerControlTo("editor"),
          title: "toggle control to the Editor",
          "aria-label": "toggle control to the Editor",
          children: /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { className: "text-xl", children: "E" })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Button,
        {
          size: "sm",
          variant: pointerController === "website" ? "default" : "outline",
          onClick: () => changePointerControlTo("website"),
          title: "toggle control to the Website",
          "aria-label": "toggle control to the Website",
          children: /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { className: "text-xl", children: "W" })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Button,
        {
          ...
          title: "Toggle to " + (interactive ? "disable" : "enable") + " interactivity mode",
          "aria-label": "toggle interactivity mode",
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(MousePointerClick, { className: "w-6 h-6" })
        }
      )
    ] })
  ] }) });
};
const root = document.getElementById("pertento-editor-floater-root");
client.createRoot(root).render(/* @__PURE__ */ jsxRuntimeExports.jsx(EditorFloater, {}));
})();